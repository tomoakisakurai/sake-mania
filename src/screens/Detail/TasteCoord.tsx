  ...
  return (
    <section className="mb-7 max-w-140 rounded-xl border border-line bg-surface px-6 py-5">
      ...
      <figure className="relative m-0 h-55 w-full rounded-lg border border-line bg-card">
        ...
        <span className="absolute h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary" style={{ left: `${vals.d.x}%`, top: `${vals.d.y}%` }} />
        <span className="absolute text-[11px] font-bold text-primary" style={{ left: `${vals.d.x}%`, top: `${vals.d.y}%`, transform: 'translate(11px, -7px)' }}>みんなの平均</span>
        {vals.dHasMyPoint && (
          <>
            <span className="absolute h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent" style={{ left: `${vals.dMyX}%`, top: `${vals.dMyY}%` }} />
            <span className="absolute text-[11px] font-bold text-accent" style={{ left: `${vals.dMyX}%`, top: `${vals.dMyY}%`, transform: 'translate(11px, -7px)' }}>あなたの打点</span>
          </>
        )}
      </figure>
    </section>
