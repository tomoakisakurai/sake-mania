...
export function Comments({ post, commentState }: { post: PostVM; commentState: CommentState }) {
  return (
    <>
      <section className="mt-6.5 flex flex-wrap items-center gap-4.5 border-t border-line-soft pt-5">
        {post.canNomi && (
          ...
          >
            ...
            のみたいね <span className="font-mono text-[12.5px]">{post.nomiCount}</span>
          </button>
        )}
        {post.cantNomi && (
          <p className="m-0 inline-flex items-center gap-2 text-[13.5px] font-bold text-accent">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M4 3h16c0 6.2-3.5 9.8-7 10.7V18h4v2.5H7V18h4v-4.3C7.5 12.8 4 9.2 4 3z"></path></svg>
            のみたいね {post.nomiCount} <span className="text-[12px] font-normal text-muted">— みんなが飲みたがっています</span>
          </p>
        )}
      </section>
      <section className="mt-5.5">
        <h2 className="m-0 mb-1 font-serif text-[16px] font-bold">コメント <span className="font-mono text-[12px] font-normal text-muted">{post.commentCount}件</span></h2>
        <ul className="m-0 p-0 list-none">
          {post.comments.map((comment, i) => (
            <CommentItem key={i} comment={comment} commentState={commentState} />
          ))}
        </ul>
        <CommentForm post={post} commentState={commentState} />
      </section>
    </>
  );
}
