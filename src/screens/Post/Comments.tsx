...
export function Comments({ v, cs }: { v: Vals; cs: CommentState }) {
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 22 }}>
        ...
        {v.post.comments.map((cm, i: number) => {
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16, alignItems: 'flex-start' }}>
          ...
          <div onClick={() => { v.post.commentSend(cs.draft); cs.setDraft(''); }} style={{ background: '#32507C', color: '#FDFBF5', borderRadius: 999, padding: '11px 24px', fontSize: 13, fontWeight: 700, cursor: 'pointer', flexShrink: 0, alignSelf: 'flex-end' }}>送る</div>
        </div>
