    ...
    <article onClick={onClick} className="cursor-pointer rounded-xl border border-line bg-card px-5 py-4 transition-colors hover:border-primary">
      <header className="mb-2 flex items-center gap-3">
        ...
        <hgroup className="min-w-0 flex-1">
          <h3 className="m-0 font-serif text-[16px] font-bold">{member.display}</h3>
          <p className="m-0 text-[11.5px] text-muted">{member.dept} ・ {member.taste}</p>
        </hgroup>
        <span className="shrink-0 text-[13px] font-bold text-primary">→</span>
      </header>
      {member.hometownNote && (
        <blockquote className="m-0 border-t border-line-soft pt-2.5 text-[12.5px] leading-relaxed text-body">「{member.hometownNote}」</blockquote>
      )}
    </article>
  );
}
