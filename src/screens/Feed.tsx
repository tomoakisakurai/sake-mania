    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: v.pagePad }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {v.feedAll.map((f: any, i: number) => (
          <div key={i} onClick={f.click} style={{ background: '#FFFFFF', border: '1px solid #E3DBCB', borderRadius: 12, padding: '22px 26px', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', gap: 18 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 2, flexWrap: 'wrap' }}>
                  {f.tags.map((t2: any, i: number) => (
                    <span key={i} style={{ background: '#F6F1E7', borderRadius: 999, padding: '3px 12px', fontSize: 11, color: '#5C5547' }}>{t2}</span>
                  ))}
                </div>
              </div>
            </div>
