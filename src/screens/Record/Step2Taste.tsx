  ...
  return (
    <>
      <div style={{ background: '#FDFBF5', border: '1px solid #E3DBCB', borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22 }}>
        ...
        <div><div style={{ fontSize: 14.5, fontWeight: 700 }}>{vals.recBrandName}</div><div style={{ fontSize: 11.5, color: '#8B8273' }}>{vals.recBrandSub}</div></div>
        <div onClick={vals.recChangeBrand} style={{ marginLeft: 'auto', fontSize: 12, color: '#32507C', fontWeight: 700, cursor: 'pointer' }}>変更</div>
      </div>
      ...
      <div onClick={vals.onMapTap} style={{ position: 'relative', width: '100%', height: 320, background: '#FFFFFF', border: '1px solid #E3DBCB', borderRadius: 8, cursor: 'crosshair', marginBottom: 8 }}>
        ...
        {vals.recGhostDots.map((g, i: number) => (
        ...
        {vals.recHasPoint && (
          <>
            <div style={{ position: 'absolute', left: `${vals.recX}%`, top: `${vals.recY}%`, transform: 'translate(-50%, -50%)', width: 38, height: 38, borderRadius: '50%', background: 'rgba(50,80,124,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
              <div style={{ width: 17, height: 17, borderRadius: '50%', background: '#32507C', boxShadow: '0 1px 6px rgba(50,80,124,0.5)' }} />
            </div>
            <div style={{ position: 'absolute', left: `${vals.recX}%`, top: `${vals.recY}%`, transform: 'translate(24px, -10px)', fontSize: 11.5, fontWeight: 700, color: '#32507C', pointerEvents: 'none' }}>この一杯</div>
          </>
        )}
      </div>
      <div style={{ fontSize: 11, color: '#A89D8A', marginBottom: 24 }}>薄い点はあなたの過去の盃。打点すると味わいの言葉に自動変換されます — <b style={{ color: '#5C5547' }}>{vals.recTasteLabel}</b></div>
      <div style={{ marginBottom: 26 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, fontWeight: 700, marginBottom: 6 }}><span>甘口</span><span style={{ color: '#8B8273', fontWeight: 400, fontSize: 11 }}>甘辛 — {vals.recSweetLabel}</span><span>辛口</span></div>
        <input type="range" min={0} max={100} value={vals.recSweet} onChange={vals.onSweet} style={{ width: '100%', accentColor: '#32507C', cursor: 'pointer' }} />
      </div>
      <div style={{ marginBottom: 8 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 8 }}>総合評価</div>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {vals.recStars.map((sr, i: number) => (
            <span key={i} onClick={sr.click} style={{ fontSize: 30, cursor: 'pointer', color: sr.color, lineHeight: 1 }}>★</span>
          ))}
          <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 14, color: '#8B8273', marginLeft: 8 }}>{vals.recRatingLabel}</span>
        </div>
      </div>
    </>
  );
