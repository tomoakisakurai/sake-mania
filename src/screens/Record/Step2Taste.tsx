...
export function Step2Taste({ recVals }: { recVals: RecordVals }) {
  return (
    <>
      <section className="mb-5.5 flex items-center gap-3.5 rounded-xl border border-line bg-surface px-5 py-4">
        ...
        <hgroup>
          <h2 className="m-0 text-[14.5px] font-bold">{recVals.recBrandName}</h2>
          <p className="m-0 text-[11.5px] text-muted">{recVals.recBrandSub}</p>
        </hgroup>
        <a onClick={recVals.recChangeBrand} className="ml-auto cursor-pointer text-[12px] font-bold text-primary">変更</a>
      </section>
      ...
      <figure onClick={recVals.onMapTap} className="relative m-0 mb-2 h-80 w-full cursor-crosshair rounded-lg border border-line bg-card">
        ...
        {recVals.recGhostDots.map((ghost, i) => (
        ...
        {recVals.recHasPoint && (
          <>
            <span className="pointer-events-none absolute flex h-9.5 w-9.5 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-primary/15" style={{ left: `${recVals.recX}%`, top: `${recVals.recY}%` }}>
              <span className="h-4.25 w-4.25 rounded-full bg-primary" style={{ boxShadow: '0 1px 6px rgba(50,80,124,0.5)' }} />
            </span>
            <span className="pointer-events-none absolute text-[11.5px] font-bold text-primary" style={{ left: `${recVals.recX}%`, top: `${recVals.recY}%`, transform: 'translate(24px, -10px)' }}>この一杯</span>
          </>
        )}
      </figure>
      <p className="m-0 mb-6 text-[11px] text-faint">薄い点はあなたの過去の盃。打点すると味わいの言葉に自動変換されます — <b className="text-body">{recVals.recTasteLabel}</b></p>
      <section className="mb-6.5">
        <header className="mb-1.5 flex justify-between text-[12.5px] font-bold">
          <span>甘口</span>
          <span className="text-[11px] font-normal text-muted">甘辛 — {recVals.recSweetLabel}</span>
          <span>辛口</span>
        </header>
        <input type="range" min={0} max={100} value={recVals.recSweet} onChange={recVals.onSweet} className="w-full cursor-pointer accent-primary" />
      </section>
      <section className="mb-2">
        <h2 className="m-0 mb-2 text-[12.5px] font-bold">総合評価</h2>
        <div className="flex items-center gap-1.5">
          {recVals.recStars.map((star, i) => (
            <span key={i} onClick={star.click} className="cursor-pointer text-[30px] leading-none" style={{ color: star.color }}>★</span>
          ))}
          <span className="ml-2 font-mono text-[14px] text-muted">{recVals.recRatingLabel}</span>
        </div>
      </section>
    </>
  );
