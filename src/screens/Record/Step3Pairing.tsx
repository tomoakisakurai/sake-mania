...
export function Step3Pairing({ recVals }: { recVals: RecordVals }) {
  ...
  return (
    <>
      ...
      <section className="mb-6">
        ...
        <Input
          value={recVals.recPairing}
          onChange={recVals.onPairing}
          placeholder="例: 白子ポン酢、鯖の塩焼き"
        />
      </section>
      <section>
        <h2 className="m-0 mb-2.5 text-[13.5px] font-bold">写真</h2>
        <input type="file" accept="image/*" ref={photoRef} onChange={recVals.onPhoto} className="hidden" />
        {recVals.recHasPhoto && (
          <figure className="relative m-0 overflow-hidden rounded-xl border border-line">
            <img src={recVals.recPhoto} alt="" className="block h-65 w-full object-cover" />
            <figcaption className="absolute right-3 bottom-3 flex gap-2">
              ...
              <button type="button" onClick={recVals.onPhotoRemove} className="cursor-pointer rounded-full bg-ink/80 px-4 py-2 text-[12px] font-bold text-bg">削除</button>
            </figcaption>
          </figure>
        )}
        {recVals.recNoPhoto && (
