...
export function Done({ registeredName, notifySlack, onAnother, onGoHome }: { registeredName: string; notifySlack: boolean; onAnother: () => void; onGoHome: () => void }) {
  return (
    <div style={{ textAlign: "center", padding: "40px 0 20px" }}>
      ...
      {notifySlack ? (
        <p className="m-0 mb-4 inline-flex items-center gap-2 rounded-full border border-success-tint-strong bg-success-tint px-4.5 py-[7px] text-[12px] font-bold text-success-dark">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
          Slack #日本酒部 に通知しました
        </p>
      ) : (
        <p className="m-0 mb-4 inline-flex items-center gap-2 rounded-full border border-line bg-bg px-4.5 py-[7px] text-[12px] font-bold text-muted">Slack通知はオフのため送信されませんでした</p>
      )}
