  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {vals.isDesktopNav && (
        <>
          {vals.homeNext ? (
            <div onClick={vals.homeNext.click} style={{ background: '#32507C', borderRadius: 12, padding: '22px 24px', color: '#FDFBF5', cursor: 'pointer' }}>
              ...
              <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 19, fontWeight: 700, lineHeight: 1.5 }}>{vals.homeNext.name}</div>
              <div style={{ fontSize: 12.5, opacity: 0.85, marginTop: 6 }}>{vals.homeNext.dateLabel} ・ {vals.homeNext.place}</div>
              <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>テーマ:{vals.homeNext.theme}</div>
              <div style={{ display: 'flex', gap: 16, marginTop: 12, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, opacity: 0.85 }}><span>{vals.homeNext.goingLabel}</span><span>{vals.homeNext.bringLabel}</span></div>
          ...
          {vals.homeVoting && (
            <div onClick={vals.homeVoting.click} style={{ background: '#FBF0E6', border: '1px solid #E8C9A8', borderRadius: 12, padding: '16px 20px', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 11, color: '#9A5A20' }}>締切 {vals.homeVoting.deadline}</span>
              </div>
              <div style={{ fontFamily: "'Shippori Mincho', serif", fontSize: 15, fontWeight: 700, color: '#2E2A24' }}>{vals.homeVoting.name}</div>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E3DBCB', borderRadius: 12, padding: '16px 20px 6px' }}>
        ...
        <div onClick={vals.openMeetupCreate} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '13px 0', cursor: 'pointer', color: '#32507C', fontSize: 12.5, fontWeight: 700 }}>
