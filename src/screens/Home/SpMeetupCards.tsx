...
export function SpMeetupCards({ home }: { home: HomeVals }) {
  return (
    <div className="md:hidden">
      {home.homeNext && (
        <article
          onClick={home.homeNext.click}
        ...
        >
          ...
          <h3 className="font-serif text-[18px] font-bold leading-[1.45] m-0">{home.homeNext.name}</h3>
          <p className="mt-1 text-[12px] opacity-85 m-0">{home.homeNext.dateLabel} ・ {home.homeNext.place}</p>
          <p className="mt-0.5 text-[11px] opacity-70 m-0">テーマ: {home.homeNext.theme}</p>
          <p className="mt-2.5 flex gap-3.5 font-mono text-[10px] opacity-85 m-0">
            <span>{home.homeNext.goingLabel}</span>
            <span>{home.homeNext.bringLabel}</span>
          </p>
      ...
      {home.homeVoting && (
        ...
        >
          <header className="mb-1 flex items-center gap-2">
            ...
            <span className="text-[10.5px] text-accent-dark">締切 {home.homeVoting.deadline}</span>
          </header>
          <h3 className="font-serif text-[14px] font-bold text-ink m-0">{home.homeVoting.name}</h3>
