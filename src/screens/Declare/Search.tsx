...
export function Search({ query, setQuery, results }: { query: string; setQuery: (q: string) => void; results: SearchResult[] }) {
  return (
    <>
      <div style={{ background: "#FFFFFF", border: "1.5px solid #E3DBCB", borderRadius: 12, padding: "13px 18px", fontSize: 14, color: "#B4B6AC", marginBottom: 16 }}>
        <input type="text" value={query} onChange={(e: ChangeEvent<HTMLInputElement>) => setQuery(e.target.value)} placeholder="銘柄・酒蔵で検索…" style={{ width: "100%", border: 0, background: "transparent", fontSize: 14, fontFamily: "'Zen Kaku Gothic New', sans-serif", color: "#2E2A24", outline: "none" }} />
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {results.map((dr, i: number) => (
