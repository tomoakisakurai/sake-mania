...
export async function getReferenceData(): Promise<ReferenceData> {
  ...
  try {
    ...
    const brands: Brand[] = bRows.map((r) => ({
      ...
      rating: r.rating, count: r.count, tags: arr(r.tags), desc: r.description,
    }));
    ...
    const others: OtherRec[] = oRows.map((r) => ({
      rid: r.rid, nomi: r.nomi, comments: arr(r.comments), user: r.user, avatar: r.avatar,
      avatarBg: r.avatarBg, time: r.time, place: r.place, brandId: r.brandId, rating: r.rating,
      x: r.x, y: r.y, sweet: r.sweet, temps: arr(r.temps), pairing: r.pairing, memo: r.memo, date: r.date,
    }));
